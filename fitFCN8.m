function [ fit ] = fitFCN8(X, ZD_func,existing_points)
...
if nconstr == 0 && nconstr_lin ==0
    ...
    closeness = zeros(length(func),1);
    for i = 1:size(existing_points,1) 
        close_val = 0.0001*((func(:,1)-existing_points(i,1)).^2 + (func(:,2)-existing_points(i,2)).^2).^-1;
        closeness(close_val > closeness) = close_val(close_val > closeness); 
    end
    fit = -func(:,sim_col) + closeness;
